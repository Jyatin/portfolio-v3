        ...
        <>
            <nav className={`fixed top-0 z-50 w-full border-b border-border/70 bg-background/85 px-7 py-4 text-foreground backdrop-blur-md transition-transform duration-300 sm:px-10 ${navHidden && !menuOpen ? "-translate-y-full" : "translate-y-0"}`}>
                <div className="mx-auto flex max-w-[1400px] items-center justify-between gap-3">
                    <Link href="/" className="text-base font-bold tracking-[-0.04em] text-foreground/70 sm:text-lg" aria-label="Jyatin Kumar Singh home">Jyatin</Link>
                    <div className="hidden items-center gap-2 md:flex">
                        {navItems.map((item) => <Link key={item.href} href={item.href} className="px-3 py-2 font-mono text-[10px] uppercase tracking-[0.2em] text-foreground/55 transition-colors hover:text-foreground">{item.name}</Link>)}
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button type="button" onClick={toggleTheme} className="flex h-10 w-10 items-center justify-center rounded-xl border border-border text-foreground/65 transition-colors hover:border-foreground/40 hover:text-foreground" aria-label={themeReady && resolvedTheme === "dark" ? "Switch to light mode" : "Switch to dark mode"}>{themeReady && resolvedTheme === "dark" ? <Sun className="h-5 w-5" /> : <Moon className="h-5 w-5" />}</button>
                        <button type="button" onClick={() => setMenuOpen((value) => !value)} className="flex h-10 w-10 items-center justify-center text-foreground/85 md:hidden" aria-label="Toggle menu" aria-expanded={menuOpen}>{menuOpen ? <X className="h-7 w-7" /> : <Menu className="h-7 w-7" />}</button>
                    </div>
                </div>
            </nav>
